...
local function showHelpMenu()
    ...
    -- END HEBREW SUPPORT
    local menuBar =
        ax.systemElementAtPosition({0, 0}):attributeValue("AXParent")
    for _, v in ipairs(menuBar) do
        if v:attributeValue("AXTitle") == "Help" then return v:doPress() end
    end
end
...
local function rightClick()
    ...
    ax.applicationElement(thisApp):focusedUIElement()
...
Hotkey.bind(hyper, "right",
            function() spoon.WindowManager.pushToCell("Right") end)
