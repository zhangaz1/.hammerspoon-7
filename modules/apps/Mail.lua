...
local function mailGetLinks()
	...
	local e = ui.getUIElement(window, ({
		{'AXSplitGroup', 1},
		{'AXSplitGroup', 1},
		{'AXScrollArea', 2},
		{'AXGroup', 1},
...
local function textCompletion()
	...
	if (writingAreaAXIdentifier ~= 'AXWebArea') or (trulyFocusedAppIdentifier ~= m.id) then
		m.modal:exit()
...
m.modal:bind({'alt'}, 'o', function() mailGetLinks() end)
m.modal:bind({'cmd'}, 'd', function() textCompletion() end)

m.appScripts = {
	{ title = 'Copy Sender Address', func = function() copySenderAddress() end }
}
