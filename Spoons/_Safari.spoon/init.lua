...
local function changeToABCAfterFocusingAddressBar(modal, keystroke)
    if KeyCodes.currentLayout() == "Hebrew" then KeyCodes.setLayout("ABC") end
    modal:exit()
...
local hotkeys = {
    {"ctrl", ",", function() moveTab("left") end},
    {"ctrl", ".", function() moveTab("right") end},
    {"cmd", "n", function() _appObj:selectMenuItem({"File", "New Window"}) end},
    {"ctrl", "i", function() goToFirstInputField() end},
    ...
    }, {"alt", "1", function() moveFocusToSafariMainArea(_appObj, true) end},
    {"alt", "2", function() moveFocusToSafariMainArea(_appObj, false) end},
    {{"cmd", "shift"}, "n", function() newBookmarksFolder(_appObj) end},
    {"alt", "r", function() rightSizeBookmarksOrHistoryColumn(_appObj) end},
    {{}, "tab", function() firstSearchResult(_appObj, _modal) end}
}
